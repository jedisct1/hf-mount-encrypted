...
pub trait HubOps: Send + Sync {
    ...
    /// the feed (repo source, encrypted mount, 404, non-SSE response): the caller falls back
...
impl RemotePath {
    ...
    /// percent-encoded per segment; plaintext paths go through
    /// `encode_url_path`, which escapes URL-reserved characters while
...
impl HubApiClient {
    ...
    pub async fn follow_events(
        ...
        if self.path_cipher.is_some() {
            debug!("live-follow: encrypted mounts use interval polling for decrypted paths and sizes");
            return Ok(None);
        }
    ...
    pub async fn list_tree(&self, prefix: &str) -> Result<Vec<TreeEntry>> {
        ...
        // The comparison runs on remote (possibly encrypted) paths, so it must
        // happen before decryption while entries and prefix share coordinates.
        if !api_prefix.is_empty() {
            entries.retain(|e| strict_descendant_rel(&e.path, api_prefix.as_body()).is_some());
        }
...
mod tests {
    ...
    fn test_resolve_url_encodes_revision() {
        ...
        assert_eq!(
            c.resolve_url("config.json").unwrap(),
    ...
    #[test]
    fn plaintext_paths_stay_raw_in_body_but_encode_in_url() {
        // A non-encrypted client keeps request bodies raw, but URLs still
        // percent-encode reserved characters (`?` would start a query string).
        let c = make_test_client("", None);
        let rp = c.prefixed_path("a b?c.txt").unwrap();
        assert_eq!(rp.as_body(), "a b?c.txt");
        assert_eq!(rp.as_url(), "a%20b%3Fc.txt");
    }
    ...
    mod encryption_boundary {
        ...
        #[tokio::test]
        async fn encrypted_clients_fall_back_to_polling_without_connecting() {
            for prefix in ["", "sub/dir"] {
                let mut c = encrypted_client(prefix);
                c.endpoint = "invalid URL".to_string();
                for (cursor, since) in [(None, None), (Some("cursor"), None), (None, Some("revision"))] {
                    assert!(c.follow_events(cursor, since).await.unwrap().is_none());
                }
                c.path_cipher = None;
                assert!(c.follow_events(None, None).await.is_err());
            }
        }
