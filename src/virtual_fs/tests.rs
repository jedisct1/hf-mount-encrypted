...
fn spawn_poll_loop(
    ...
    #[cfg(feature = "encrypt")]
    let xet = vfs.xet_sessions.clone();
    #[cfg(feature = "encrypt")]
    let encryption = vfs.encryption.clone();
    #[cfg(feature = "encrypt")]
    let read_fetch_timeout = vfs.read_fetch_timeout;
...
mod encryption {
    ...
    #[test]
    fn follow_fallback_repairs_encrypted_file_and_reads_new_content() {
        let enc = encryptor();
        let original = build_object(&enc.content_key, b"original content");
        let replacement: Vec<u8> = (0..70_000).map(|i| (i % 251) as u8).collect();
        let replacement_object = build_object(&enc.content_key, &replacement);
        let replacement_cipher_size = replacement_object.len() as u64;
        let hub = MockHub::new();
        let xet = MockXet::new();
        xet.add_file("h1", &original);
        xet.add_file("h2", &replacement_object);
        hub.add_file("secret.bin", original.len() as u64, Some("h1"), None);
        hub.set_revision("rev-a");
        let rt = super::new_runtime();
        let vfs = make_encrypted_test_vfs(hub.clone(), xet, enc, opts(), &rt);

        rt.block_on(async {
            let attr = vfs.lookup(ROOT_INODE, "secret.bin").await.unwrap();
            assert_eq!(attr.size, b"original content".len() as u64);
            let (stop, handle) = super::spawn_poll_loop(&vfs, &hub, true);
            assert!(super::wait_until(|| hub.probe_revision_call_count() >= 2).await);

            hub.remove_file("secret.bin");
            hub.add_file("secret.bin", replacement_cipher_size, Some("h2"), None);
            hub.set_revision("rev-b");
            let repaired = super::wait_until(|| {
                let inodes = vfs.inode_table.read().unwrap();
                let entry = inodes.get(attr.ino).unwrap();
                entry.xet_hash.as_deref() == Some("h2")
                    && entry.cipher_size == Some(replacement_cipher_size)
                    && entry.size == replacement.len() as u64
                    && !entry.needs_size_probe
            })
            .await;
            stop.notify_one();
            handle.await.unwrap();
            assert!(repaired, "poll fallback must repair encrypted metadata");

            assert_eq!(vfs.getattr(attr.ino).unwrap().size, replacement.len() as u64);
            let fh = vfs.open(attr.ino, false, false, Some(1)).await.unwrap();
            let (data, eof) = vfs.read(fh, 0, replacement.len() as u32 + 1).await.unwrap();
