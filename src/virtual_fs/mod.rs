...
impl VirtualFs {
        ...
        match fast {
            ...
            FastResult::Miss { full_path, local_only } => {
                ...
                match self.head_probe(&full_path).await {
                    HeadProbe::File(head) => return self.insert_file_from_head(parent, name, &full_path, head).await,
            ...
            let result: std::result::Result<Vec<u8>, crate::error::Error> = async {
                ...
                loop {
                    ...
                    let chunk = match read_fetch_timeout {
                        ...
                        timeout => tokio::time::timeout(timeout, stream.next()).await.map_err(|_| {
                            crate::error::Error::hub(format!("stream read timed out after {timeout:?}"))
                        })??,
                    };
                    match chunk {
    ...
    async fn probe_plaintext_size_with(
        ...
        let header =
            Self::fetch_object_range_with(xet, &file_info, 0, content::CONTAINER_HEADER_LEN, read_fetch_timeout)
                .await
                .ok()?;
    ...
    async fn probe_remote_plaintext_size(&self, xet_hash: &str, cipher_size: u64) -> Option<u64> {
        ...
        Self::probe_plaintext_size_with(
            &*self.xet_sessions,
            encryptor,
            xet_hash,
            cipher_size,
    ...
    fn link_source(&self, ino: u64) -> VirtualFsResult<Option<LinkSource>> {
        ...
        Ok(Some(LinkSource {
            ...
            xet_hash: hash,
            size: source.size,
            cipher_size: source.cipher_size,
            mode: source.mode,
            uid: source.uid,
            gid: source.gid,
        }))
    ...
    pub async fn link(&self, ino: u64, newparent: u64, newname: &str) -> VirtualFsResult<VirtualFsAttr> {
        ...
        if let Some(cs) = source.cipher_size
